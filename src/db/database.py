...
def fetch_denied_cancellation_ticket(order_id: str) -> dict | None:
    query = (
        select(support_tickets_table)
        .where(
            support_tickets_table.c.order_id == order_id,
            support_tickets_table.c.issue_type == "order_cancellation",
            support_tickets_table.c.status == "DENIED",
        )
        .order_by(support_tickets_table.c.ticket_id.desc())
    )
